...
### ### ###
    ...
    # opensource.references.add(postgre, mariadb, mssqlserver, flockdb, cassandra, hbase, foundationdb, leveldb, riak, couchdb, tokumx)
...
### NoSQL ###
    ...
    #                   oracle, virtuoso, orientdb, mongodb, cassandra, hbase, dynamodb, aerospike, ignite, foundationdb, hazelcast,
    #                   hibari, infinitydb, leveldb, oraclenosql, voldemort, redis, riak, rocksdb, basex, cloudant, couchdb, simpledb, tokumx)
...
# Others
    ...
    keyvalue.text = "A key–value database, or key–value store, is a data storage paradigm designed for storing, retrieving, and managing associative arrays, and a data structure more commonly known today as a dictionary or hash table. Dictionaries contain a collection of objects, or records, which in turn have many different fields within them, each containing data. These records are stored and retrieved using a key that uniquely identifies the record, and is used to find the data within the database."
    db.graph.push(keyvalue)
    # keyvalue.references.add(aerospike, ignite, arangodb, foundationdb, hazelcast, hibari, infinitydb, leveldb, oraclenosql, voldemort, redis, riak, rocksdb, virtuoso)

    redis = Article()
    redis.title = "Redis"
    redis.text = "Redis (Remote Dictionary Server) is an in-memory data structure store, used as a distributed, in-memory key–value database, cache and message broker, with optional durability. Redis supports different kinds of abstract data structures, such as strings, lists, maps, sets, sorted sets, HyperLogLogs, bitmaps, streams, and spatial indices."
    db.graph.push(redis)
    # redis.references.add(nosql, keyvalue)

    aerospike = Article()
    aerospike.title = "Aerospike"
    aerospike.text = "Aerospike is a flash-optimized and in-memory open source distributed key value NoSQL database, and the name of the eponymous company that produces it."
    db.graph.push(aerospike)
    # aerospike.references.add(nosql, mongodb)

    ignite = Article()
    ignite.title = "Apache Ignite"
    ignite.text = "Apache Ignite is a distributed database for high-performance computing with in-memory speed. Apache Ignite's database utilizes RAM as the default storage and processing tier, thus, belonging to the class of in-memory computing platforms. The disk tier is optional but, once enabled, will hold the full data set whereas the memory tier will cache full or partial data set depending on its capacity."
    db.graph.push(ignite)
    # ignite.references.add(nosql, keyvalue)

    foundationdb = Article()
    foundationdb.title = "FoundationDB"
    foundationdb.text = "FoundationDB is a free and open-source multi-model distributed NoSQL database developed by Apple Inc. with a shared-nothing architecture. The product was designed around a 'core' database, with additional features supplied in 'layers'."
    db.graph.push(foundationdb)
    # foundationdb.references.add(nosql, keyvalue, opensource)

    hazelcast = Article()
    hazelcast.title = "Hazelcast"
    hazelcast.text = "In computing, Hazelcast IMDG is an open source in-memory data grid based on Java. In a Hazelcast grid, data is evenly distributed among the nodes of a computer cluster, allowing for horizontal scaling of processing and available storage."
    ...
    infinitydb.text = "InfinityDB is an all-Java embedded database engine and client/server DBMS with an extended java.util.concurrent.ConcurrentNavigableMap interface that is deployed in handheld devices, on servers, on workstations, and in distributed settings. The design is based on a proprietary lockless, concurrent, B-tree architecture that enables client programmers to reach high levels of performance without risk of failures."
    db.graph.push(infinitydb)
    # infinitydb.references.add(nosql, keyvalue)

    leveldb = Article()
    leveldb.title = "LevelDB"
    leveldb.text = "LevelDB is an open-source on-disk key-value store written by Google fellows Jeffrey Dean and Sanjay Ghemawat. Inspired by Bigtable, LevelDB is hosted on GitHub under the New BSD License and has been ported to a variety of Unix-based systems, and macOS, Windows, and Android."
    db.graph.push(leveldb)
    # leveldb.references.add(nosql, keyvalue, opensource)

    voldemort = Article()
    voldemort.title = "Voldemort (distributed data store)"
    voldemort.text = "Voldemort is a distributed data store that was designed as a key-value store used by LinkedIn for highly-scalable storage. It is named after the fictional Harry Potter villain Lord Voldemort."
    db.graph.push(voldemort)
    # keyvalue.references.add(nosql, keyvalue)

    riak = Article()
    riak.title = "Riak"
    riak.text = "Riak is a distributed NoSQL key-value data store that offers high availability, fault tolerance, operational simplicity, and scalability. In addition to the open-source version, it comes in a supported enterprise version and a cloud storage version."
    db.graph.push(riak)
    # riak.references.add(nosql, keyvalue, opensource, dynamodb, leveldb)

    rocksdb = Article()
    rocksdb.title = "RocksDB"
    rocksdb.text = "RocksDB is a high performance[2][3][4][5][6] embedded database for key-value data. It is a fork of Google's LevelDB optimized to exploit many CPU cores, and make efficient use of fast storage, such as solid-state drives (SSD), for input/output (I/O) bound workloads."
    db.graph.push(rocksdb)
    # rocksdb.references.add(nosql, keyvalue, leveldb)

    oraclenosql = Article()
    oraclenosql.title = "Oracle NoSQL Database"
    oraclenosql.text = "Oracle NoSQL Database (ONDB) is a NoSQL-type distributed key-value database from Oracle Corporation. It provides transactional semantics for data manipulation, horizontal scalability, and simple administration and monitoring."
    ...
    documentdb.text = "A document-oriented database, or document store, is a computer program and data storage system designed for storing, retrieving and managing document-oriented information, also known as semi-structured data."
    db.graph.push(documentdb)
    # documentdb.references.add(nosql, graphdb, keyvalue, relationaldb, mongodb, aerospike, allegro, arangodb, cosmosdb, marklogic, virtuoso,
    # oraclenosql, postgre, basex, cloudant, couchdb, simpledb, tokumx)

    basex = Article()
    basex.title = "BaseX"
    basex.text = "BaseX is a native and light-weight XML database management system and XQuery processor, developed as a community project on GitHub. It is specialized in storing, querying, and visualizing large XML documents and collections."
    db.graph.push(basex)
    # basex.references.add(nosql, documentdb)

    cloudant = Article()
    cloudant.title = "Cloudant"
    cloudant.text = "Cloudant is an IBM software product, which is primarily delivered as a cloud-based service. Cloudant is a non-relational, distributed database service of the same name. Cloudant is based on the Apache-backed CouchDB project and the open source BigCouch project."
    db.graph.push()
    # cloudant.references.add(nosql, documentdb, couchdb)

    couchdb = Article()
    couchdb.title = "Apache CouchDB"
    couchdb.text = "Apache CouchDB is an open-source document-oriented NoSQL database, implemented in Erlang. CouchDB uses multiple formats and protocols to store, transfer, and process its data. It uses JSON to store data, JavaScript as its query language using MapReduce, and HTTP for an API."
    db.graph.push(couchdb)
    # couchdb.references.add(nosql, documentdb)

    simpledb = Article()
    simpledb.title = "Amazon SimpleDB"
    simpledb.text = "Amazon SimpleDB is a distributed database written in Erlang by Amazon.com. It is used as a web service in concert with Amazon Elastic Compute Cloud (EC2) and Amazon S3 and is part of Amazon Web Services. It was announced on December 13, 2007."
    db.graph.push()
    # simpledb.references.add(nosql, documentdb, dynamodb)

    tokumx = Article()
    tokumx.title = "TokuMX"
    tokumx.text = "TokuMX is an open-source distribution of MongoDB which, among other things, replaces the default B-tree data structure found in the basic MongoDB distribution with a Fractal Tree index."
    db.graph.push()
    ...
    oraclenosql.references.add(nosql, keyvalue)
    # Document
    documentdb.references.add(nosql, graphdb, keyvalue, relationaldb, mongodb, aerospike, allegro, arangodb, cosmosdb, marklogic, virtuoso, oraclenosql, postgre, basex, cloudant, couchdb, simpledb, tokumx)
    basex.references.add(nosql, documentdb)
    cloudant.references.add(nosql, documentdb, couchdb)
    couchdb.references.add(nosql, documentdb)
    simpledb.references.add(nosql, documentdb, dynamodb)
